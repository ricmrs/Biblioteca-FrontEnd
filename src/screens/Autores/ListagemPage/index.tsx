...
export default function ListagemPage() {
  ...
  async function excluir(id: number, nome: string) {
    try {
      ...
      if (autores?.length === 1 && !primeiraPagina) {
        const numPage = (parseInt(page as string) - 1).toString();
        ...
        >
          ...
          <Box styleSheet={{ width: "100%" }}>
            ...
            <Box styleSheet={{ gap: { xs: 8, md: 10 }, height: { xs: 378, md: 463 } }}>
              {(autores == undefined) && <Carregando colorVariant="warning" />}
              {(autores?.length === 0 ? "Parece que não há nada aqui" : "")}
              {autores.map(autor =>
                <Box
                  key={autor.id}
                  styleSheet={{
                  ...
                  <Box styleSheet={{ flexDirection: "row", gap: 15 }}>
                    ...
                    <Button onClick={() => excluir(autor.id, autor.nome)} styleSheet={{ width: { xs: 25, md: 25 }, height: { xs: 25, md: 25 }, alignSelf: "center", focus: { border: "2px dotted #FFF" } }} colorVariantEnabled={false}>
